...
$("#expenses-form").on("submit", function (event) {
  ...
  var newExpenses = {
    ...
    Type: $("#expenses-type").attr("data-expense"),
  };
  ...
  $.post("/api/incm_exp", newExpenses)
    .then(function (data) {
      ...
      var th5 = $('<th></th>').addClass('bar').text("ExpenseType");
      console.log(th1)
      table.append(row);
      row.append(th1);
      row.append(th2);
      ...
      var td5 = $('<td></td>').addClass('bar').text(newExpenses.Type);
      table.append(row1);
      row1.append(td1);
      row1.append(td2);
      row1.append(td3);
  ...
  $.get("/api/incm_exp", function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      $("#exp-well-" + i).append("<h6>Type: " + data[i].type + "</h6>");
    }
  });
